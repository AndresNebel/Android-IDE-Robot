...
function sendBlockToServer()
{
	$("#ScriptText").click();
	
	var infoBloques = $('.scripts_text_view')[0].childNodes[0].innerHTML.replace(/\<span class="number">/g,'').replace(/\<span class="keyword">/g,'').replace(/\<\/span>/g,'');
	...
	$.ajax({ type: "POST",
        ...
        cache: false,
        error: function() { alert("Uh! algo no encaro ahi.."); },
        success: function(xml) {
            alert("El servidor respondio: ok!");        
        }
});
}
